...
interface SleepDefaultChartProps {
  rawData: SleepArray | null;
}

export function SleepMonthlyChartComponent({
  rawData,
}: SleepDefaultChartProps) {
  const chartRef = useRef<Chart | null>(null); // Ref to store the chart instance
  const [sortedData, setSortedData] = useState<SleepArray | []>(rawData ?? []);
    ...
    if (ctx) {
      chartRef.current = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: "Monthly sleep data (last 12 months)",
              data,
              fill: false,
...
export default SleepMonthlyChartComponent;
